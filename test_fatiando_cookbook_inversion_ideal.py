...
solver =x basin2d.Trapezoidal(x, z, gz, verts[0:2], density=-100).config(
...
mpl.show()
#mpl.savefig('testgrav.png')   # save the figure to file
# mpl.close()

### modified the verticies but do not update the model
import numpy
from fatiando import utils
from fatiando.mesher import Polygon
from fatiando.gravmag import talwani, basin2d
...
from fatiando.gravmag import talwani, basin2d
from fatiando.vis import mpl
verts = [(10000, 1.), (50000, 3000.), (90000, 1000.), (90000, 7000), (50000, 5500.), (10000, 3330)]
model = Polygon(verts, {'density': -100})
x = numpy.arange(0., 100000., 1000.)
z = numpy.zeros_like(x)
gz = utils.contaminate(talwani.gz(x, z, [model]), 0.5)
...
solver = basin2d.Trapezoidal(x, z, gz, verts[0:2], density=-100).config(
    'levmarq', initial=[9000, 500]).fit() # verts[0:len(verts)/2]
estimate = solver.estimate_
mpl.figure()
mpl.subplot(2, 1, 1)
mpl.title("Gravity anomaly")
